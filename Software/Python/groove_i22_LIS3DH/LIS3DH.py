...
class LIS3DH:
   ...
   def getAxis(self, axis):
      ...
      low = self._device.readU8(base) # Read the first register (lower bits)
      high = self._device.readU8(base + 1) # Read the next register (higher bits)
   ...
   def getRange(self):
      val = self._device.readU8(self.REG_CTRL4) # Get value from register
   ...
   def setAxisStatus(self, axis, enable):
       ...
       current = self._device.readU8(self.REG_CTRL1)
   ...
   def setClick(self,clickmode,clickthresh=80,timelimit=10,timelatency=20,timewindow=100,mycallback=None):
        if (clickmode==self.CLK_NONE):
            val = self._device.readU8(self.REG_CTRL3) # Get value from register
...
# set timing parameters
   ...
   def getClick(self):
        reg = self._device.readU8(self.REG_CLICKSRC)       # read click register
        self._device.readU8(self.REG_INT1SRC)              # reset  interrupt flag
        return reg 

   # Set the rate (cycles per second) at which data is gathered

   def setDataRate(self, dataRate):
      val = self._device.readU8(self.REG_CTRL1) # Get current value
   ...
   def setHighResolution(self, highRes=True):
      val = self._device.readU8(self.REG_CTRL4) # Get current value
